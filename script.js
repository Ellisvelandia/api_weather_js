$(function() {
    ...
    const getWeather = async(city) => {
        ...
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=d86fa358a80e6dfd27f16993aed89607`
        );
        ...
        if (!response.ok) {
            ...
            swal(
                "Oops!",
                `An error has occurred. Such city wasn't found.\nPlease write the correct city name you are looking for.`,
                "error"
            );
            isAnimated = false;
            return;
        ...
        sunriseMinutesUTC =
            timezone > 0 && timezone.toString().includes(".5") ?
            sunriseMinutesUTC + 30 :
            sunriseMinutesUTC;
        sunsetMinutesUTC =
            timezone > 0 && timezone.toString().includes(".5") ?
            sunsetMinutesUTC + 30 :
        ...
        sunriseMinutesUTC =
            timezone > 0 && timezone.toString().includes(".75") ?
            sunriseMinutesUTC + 45 :
            sunriseMinutesUTC;
        sunsetMinutesUTC =
            timezone > 0 && timezone.toString().includes(".75") ?
            sunsetMinutesUTC + 45 :
        ...
        sunriseMinutesUTC =
            sunriseMinutesUTC >= 60 ? sunriseMinutesUTC - 60 : sunriseMinutesUTC;
        sunsetMinutesUTC =
            sunsetMinutesUTC >= 60 ? sunsetMinutesUTC - 60 : sunsetMinutesUTC;

        let sunriseTime = sunriseHoursUTC + timezone;
        let sunsetTime = sunsetHoursUTC + timezone;

        sunriseTime =
            timezone.toString().includes(".5") || timezone.toString().includes(".75") ?
            Math.trunc(sunriseTime) :
            sunriseTime;
        sunsetTime =
            timezone.toString().includes(".5") || timezone.toString().includes(".75") ?
            Math.trunc(sunsetTime) :
            sunsetTime;

        sunriseTime =
            (sunriseMinutesUTCBeforeCorrect > 30 &&
                timezone.toString().includes(".5")) ||
            (sunriseMinutesUTCBeforeCorrect > 30 &&
                timezone.toString().includes(".75")) ?
            Math.trunc(sunriseTime + 1) :
            sunriseTime;
        sunsetTime =
            (sunsetMinutesUTCBeforeCorrect > 30 &&
                timezone.toString().includes(".5")) ||
            (sunsetMinutesUTCBeforeCorrect > 30 &&
                timezone.toString().includes(".75")) ?
            Math.trunc(sunsetTime + 1) :
            sunsetTime;
        ...
        sunriseTime =
            sunriseTime > 24 && sunriseTime <= 33 ? "0" + sunriseTime : sunriseTime;

        sunriseMinutesUTC =
            sunriseMinutesUTC < 10 ? "0" + sunriseMinutesUTC : sunriseMinutesUTC;
        sunsetMinutesUTC =
            sunsetMinutesUTC < 10 ? "0" + sunsetMinutesUTC : sunsetMinutesUTC;
        ...
        putData = () => {
            ...
            elements[0].html(
                isUndefined(
                    `${json.name}, ${isUndefined(country)} ${weatherIcon[0].outerHTML}`
                )
            );
            elements[1].text(`Longitude: ${isUndefined(lon)}`);
            elements[2].text(`Latitude: ${isUndefined(lat)}`);
            elements[3].text(
                `Pressure: ${isUndefined(translateTommHg(json.main.pressure))}`
            );
            ...
            elements[15].text(
                `Visibility: ${isUndefined((json.visibility / 1000).toFixed(2))} km`
            );
            elements[16].text(
                `Sunrise: ${isUndefined(sunriseTime)}:${sunriseMinutesUTC}`
            );
            elements[17].text(
                `Sunset: ${isUndefined(sunsetTime)}:${sunsetMinutesUTC}`
            );
        ...
        if (hoursNow > sunsetTime || hoursNow < sunriseTime) {
            weatherIcon[0].src = weather.main.includes("Cloud") ?
                weatherIconUrls[12] :
                weather.main.includes("Clear") ?
                weatherIconUrls[11] :
                weather.description.includes("rain") ||
                weather.main.includes("Drizzle") ||
                weather.description.includes("shower") ?
                weatherIconUrls[14] :
                weather.description.includes("overcast") ?
                weatherIconUrls[13] :
                weather.main.includes("Snow") ?
                weatherIconUrls[15] :
                weather.main.includes("Squall") ?
                weatherIconUrls[17] :
                weather.description.includes("thunderstorm") ?
                weatherIconUrls[16] :
                weather.main.includes("Haze") ||
                weather.main.includes("Smoke") ||
                weather.main.includes("Fog") ||
                weather.main.includes("Mist") ?
                weatherIconUrls[2] :
                "";

            backgroundImage.attr(
                "src",
                weather.main.includes("Cloud") ?
                gifs[14] :
                weather.main.includes("Clear") ?
                gifs[16] :
                weather.description.includes("rain") ?
            ...
            weatherIcon[0].src = weather.main.includes("Cloud") ?
                ...
                weather.main.includes("Mist") ?
                weatherIconUrls[2] :
                weather.main.includes("Tornado") ?
                weatherIconUrls[7] :
                weather.main.includes("Ash") ?
                weatherIconUrls[8] :
                weather.main.includes("Squall") ?
                weatherIconUrls[10] :
                weather.main.includes("Sand") || weather.main.includes("Dust") ?
                weatherIconUrls[9] :
                "";
            ...
            backgroundImage.attr(
                ...
                weather.main.includes("Squall") ?
                gifs[10] :
                weather.main.includes("Sand") ?
                gifs[11] :
                weather.main.includes("Dust") ?
                gifs[12] :
                weather.main.includes("Drizzle") ?
                gifs[13] :
                ""
        ...
        timer = setInterval(() => {
            ...
            if (k === 300) {
                topWrapper.css(
                    "background",
                    `linear-gradient(-270deg, #424242 ${i}%, #d2d2d2 ${j}%)`
                );
                min_temp.css(
                    "background",
                    `linear-gradient(-270deg, #424242 ${i}%, #d2d2d2 ${j}%)`
    ...
    $("#button").on("click", function() {
        ...
        const currentCityName = elements[0][0].textContent
            .toUpperCase()
            .replaceAll(" ", "")
            .split(",")
            .join("");
        const searchingCityName = searchInput
            .val()
            .toUpperCase()
            .replaceAll(" ", "")
            .split(",")
    ...
    setTimeout(getWeather, 0, "carmen de apicala");
});